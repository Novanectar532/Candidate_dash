  ...
  return (
    ...
    <div className="ml-0 p-4 bg-zinc-100">
      ...
      <div className={tableMarginTop}>
              ...
              {applications.map((app, index) => (
                <tr key={index} className="border-b">
                  <td className="py-3 flex items-center gap-2 pl-3">
                    <img className="w-10 h-10 object-contain" src={app.logo} alt="" />
                    <div>
        ...
        <div className="flex flex-wrap gap-2 md:hidden mt-4 space-y-4 justify-center">
          {applications.map((app, index) => (
            <div key={index} className="border rounded-lg p-4 shadow-sm bg-white">
              ...
              <div className="text-sm text-gray-700 mb-1"><span className="font-medium">Location:</span> {app.location}</div>
              <div className="text-sm text-gray-700 mb-1"><span className="font-medium">Applied Date:</span> {app.appliedDate}</div>
              <div className="text-sm text-gray-700 mb-2"><span className="font-medium">Job Role:</span> {app.jobRole}</div>
              <div className="flex items-center mt-2 gap-4">
                <button className={`text-sm px-3 py-1 rounded-full ${app.statusColor}`}>
                  {app.status}
                </button>
                <button className="text-sm px-4 py-2 border rounded-md text-blue-600 border-blue-600 hover:bg-blue-50">
                  View Detail
